...
// Renders navigation buttons for the sidebar
const MenuButton: React.FC = () => {
    ...
    // Use window.location.pathname to highlight the active menu item
    return (
        <div className="flex flex-col gap-2 items-start w-full">
            {button.map((item, index) => (
                <Link
                    to={item.link}
                    key={index}
                    className={`flex items-center w-full gap-2 px-3 py-2 text-sm rounded
